...
const validateNumericConfig = (): void => {
    ...
    if (isNaN(fetchInterval) || fetchInterval <= 0) {
        throw new Error(
            `Invalid FETCH_INTERVAL: ${process.env.FETCH_INTERVAL}. Must be a positive number (in seconds).`
        );
    }
...
export const ENV = {
    ...
    // Slippage tolerance for hourly markets (default: 15% = skip if market moved 15%+ from trader's entry)
    MAX_SLIPPAGE_PERCENT: parseFloat(process.env.MAX_SLIPPAGE_PERCENT || '15'),
};
